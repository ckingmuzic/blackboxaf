...
class BrandScrubber:
    ...
    Usage:
        ...
        scrubber.add_terms(["AffiniPay", "ClientPay", "CPACharge", "CASEpeer"])
        # OR auto-detect from field names:
        scrubber.auto_detect_brands(field_names)

        clean_text = scrubber.scrub("AffiniPay_Customer_Status__c")
    ...
    def auto_detect_brands(self, all_field_names: list[str],
        ...
        - CamelCase compound words (AffiniPay, ClientPay, CASEpeer)
        ...
        # Deduplicate case variations (CASEpeer vs CASEPeer)
...
def _looks_like_brand_name(term: str) -> bool:
    ...
    - CamelCase inner capitals (AffiniPay, ClientPay)
    - ALLCAPS-to-lowercase (CPACharge, CASEpeer)
    ...
    # Brand names have unusual sub-words: Affini+Pay, Client+Pay, User+Gem
...
def anonymize_field_name(field_name: str) -> str:
    ...
    Examples:
        AffiniPay_Customer_Status__c -> Brand_A_Customer_Status__c
        ClientPay_Product_Tier__c    -> Brand_B_Product_Tier__c
    """
    scrubber = get_scrubber()
    if scrubber:
